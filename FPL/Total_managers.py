...
async def fetch_bootstrap_data(session: ClientSession):
    ...
    async with session.get(f'{base_url}bootstrap-static/') as response:
        ...
        )  # Will raise an HTTPError if response code isn't 200
        return await response.json()


async def get_manager_details(session: ClientSession,
                              manager_id: int,
                              retries: int = 0):
    ...
    try:
        async with session.get(url, timeout=10) as response:
            ...
            return {
                'Manager':
                f"{data['player_first_name']} {data['player_last_name']}",
                'ID': data['id'],
        ...
        if retries < MAX_RETRIES:
            ...
            return await get_manager_details(session, manager_id,
                                             retries + 1)  # Retry recursively
        print(f"Error fetching data for manager ID {manager_id}: {e}")
        return {'Manager': None, 'ID': manager_id, 'Username': None}


async def fetch_batch_managers(session: ClientSession, start_id: int,
                               end_id: int):
...
async def fetch_all_managers(total_players: int):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while batch_start <= total_players:
            ...
            batch_results = await fetch_batch_managers(session, batch_start,
                                                       batch_end)
            manager_data = pd.DataFrame(batch_results)

            # Append the batch data to CSV after each batch finishes
            manager_data.to_csv(
                './data/managers.csv',
                mode='a',
                index=False,
                header=not pd.io.common.file_exists('./data/managers.csv'))
            print(f"Batch {batch_start} to {batch_end} appended to CSV.")

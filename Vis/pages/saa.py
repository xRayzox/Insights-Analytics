...
if selected_display == 'Fixture Difficulty Rating':
    ...
    fixtures_df['team_a_short'] = fixtures_df['team_a'].map(lambda x: team_short_name_mapping[teams_df[teams_df.team_name == x].id.values[0]] if x in team_name_mapping.values() else None)
    fixtures_df['team_h_short'] = fixtures_df['team_h'].map(lambda x: team_short_name_mapping[teams_df[teams_df.team_name == x].id.values[0]] if x in team_name_mapping.values() else None)
